...
import dev.tauri.jsgcore.utils.math.*;
 ...
 *
 * @author MrJake222 - edited by MineDragonCZ_
 */
public class StargateClassicSpinHelper implements ISpinHelper {
  ...
  public StargateClassicSpinHelper(AbstractSymbolType symbolType, SymbolInterface currentSymbol, EnumSpinDirection spinDirection, boolean isSpinning, SymbolInterface targetRingSymbol, long spinStartTime, int plusRounds) {
  ...
  private float calculate(float tick) {
    if (tick < 0) {
      Logging.warn("Negative argument");
      return 0;
    }
  ...
  public void toBytes(ByteBuf buf) {
    String symbolTypeId = symbolType.getId();

    buf.writeInt(symbolTypeId.length());
    buf.writeCharSequence(symbolTypeId, StandardCharsets.UTF_8);
  ...
  public void fromBytes(ByteBuf buf) {
    ...
    AbstractSymbolsManager manager = SymbolsRegistry.managerBySymbolType(buf.readCharSequence(size, StandardCharsets.UTF_8).toString());

    if(manager == null) return;
    symbolType = manager.getSymbolType();

    isSpinning = buf.readBoolean();
    currentSymbol = manager.byId(buf.readInt());
    direction = EnumSpinDirection.valueOf(buf.readInt());

    spinStartTime = buf.readLong();
    targetSymbol = manager.byId(buf.readInt());
